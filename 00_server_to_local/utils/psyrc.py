...
def redcap_fix_typos(RC):
    ...
    RC = RC.set_value(RC.scan_id=='S3402BBF','scan_id','S3402BFF')
    RC = RC.set_value(RC.scan_id=='S7191EIM','scan_id','S719EIM')
    RC = RC.set_value(RC.scan_id=='S7854VNC','scan_id','S7854NC')
    RC = RC.set_value(RC.scan_id=='S9630QVW','scan_id','S4630QVW')
    # see Imaging_dataabse_summary_03242017 - Joke Questions Addressed
    RC = RC.set_value(RC.scan_id=='S5833EJE','scan_id','S5833ESE')
    return RC
...
def redcap_merge(PSYDB,RC):
    ...
    for idx,row in RC.iterrows():
        ...
        if not pd.isnull(row.subject_id) and row.subject_id in PSYDB.subject_id.values:
            # check index
